...
function rejects($fn)
{
    try {
        ...
        return;
    }throw new RuntimeException('Accepted invalid input');
}
$root = sys_get_temp_dir() . '/edit-html-config-' . bin2hex(random_bytes(8));
mkdir($root);
try {
    ...
    foreach ($payloads as $html) {
        ...
        check(!preg_match('/<\/?(?:script|svg|math|iframe|object|form|input|button)\b|\bon\w+\s*=|javascript:|expression\(|data:image/i', $clean), 'Unsafe HTML survived: ' . $clean);
        check(HtmlSanitizer::clean($clean) === $clean, 'Not idempotent');
    }
    ...
    $active = $root . '/.versions/' . $first['generation'];
    ...
    $pointer = file_get_contents($root . '/.active.json');
    rename($root . '/backups', $root . '/saved-backups');
    file_put_contents($root . '/backups', 'blocked');
    set_error_handler(static fn () => true);
    try {
        $store->apply(['blocks/valid.json' => json_encode(['key' => 'valid', 'label' => 'Valid', 'fields' => [['key' => 'text', 'type' => 'text']]])]);
        throw new LogicException('Missing backup accepted');
    ...
    unlink($root . '/backups');
    rename($root . '/saved-backups', $root . '/backups');
    $bad = $root . '/bad.zip';
    $zip = new ZipArchive();
    $zip->open($bad, ZipArchive::CREATE);
    $zip->addFromString('../escape.json', $json);
    $zip->close();
    rejects(fn () => Store::readZip($bad));
    unlink($bad);
    $zip = new ZipArchive();
    $zip->open($bad, ZipArchive::CREATE);
    $zip->addFromString('modules/link.json', $json);
    $zip->setExternalAttributesName('modules/link.json', ZipArchive::OPSYS_UNIX, 0120777 << 16);
    $zip->close();
    rejects(fn () => Store::readZip($bad));
    $badModule = $module;
    $badModule['field_groups'][0]['fields'][1]['config']['fields'][] = ['key' => 'body', 'type' => 'text'];
    rejects(fn () => Validator::validate(['modules/property.json' => json_encode($badModule)]));
