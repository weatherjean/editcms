...
function rejected($fn){try{$fn();}catch(DomainException|InvalidArgumentException $e){return;}throw new RuntimeException('Unexpected success');}
use Edit\Core\Database\Database;
use Edit\Core\Auth\{Auth,Installation,Permissions};
$db=new Database(':memory:');$setup=new Installation($db);$auth=new Auth($db);
$claim=fn($code)=>$setup->claim($code,'admin@example.invalid','FixturePassword789!','Admin');
rejected(fn()=> $claim(''));
$first=$setup->issueToken();$token=$setup->issueToken();rejected(fn()=> $claim($first));
check(!str_contains(json_encode($db->table('settings')->get()),$token),'Raw setup token persisted');
$db->table('settings')->where('key','setup_token_expires')->update(['value'=>'1']);rejected(fn()=> $claim($token));
$token=$setup->issueToken();$admin=$claim($token);rejected(fn()=> $claim($token));
check($db->table('settings')->where('key','setup_token_hash')->count()===0,'Token not consumed');
try{$setup->issueToken();throw new LogicException('Claimed installation issued code');}catch(RuntimeException $e){}
...
rejected(fn()=> $auth->changeRole($admin,'editor'));rejected(fn()=> $auth->deleteUser($admin));
$auth->changeRole($editor,'admin');check($auth->verifyToken($editorToken)===null,'Role change did not revoke sessions');
$auth->changeRole($admin,'editor');rejected(fn()=> $auth->changeRole($editor,'editor'));rejected(fn()=> $auth->deleteUser($editor));
$auth->deleteUser($admin);check($db->table('users')->count()===1,'Account deletion failed');
...
foreach ([['GET','/post-types'],['GET','/field-groups'],['GET','/blocks'],['GET','/auth/me'],['POST','/property'],['PUT','/property/1'],['DELETE','/property/1'],['POST','/property/1/revisions/1/restore'],['POST','/media']] as [$method,$path]) check(Permissions::allows('editor',$method,$path,['property']),'Editor workflow blocked: '.$path);
...
try{
 $old=new Database($file);$old->table('users')->insert(['email'=>'legacy@example.invalid','password'=>'fixture','name'=>'Legacy']);
 $old->execute('ALTER TABLE users DROP COLUMN role');unset($old);
 $upgraded=new Database($file);check($upgraded->table('users')->first()['role']==='admin','Legacy user lost privileges');
 unset($upgraded);
}finally{unlink($file);}
