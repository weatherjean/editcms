...
$db = new InterleavingDatabase(':memory:');
$auth = new Edit\Core\Auth\Auth($db);
...
$a = $auth->login('one@example.invalid', 'FixturePassword789!')['token'];
$b = $auth->login('one@example.invalid', 'FixturePassword789!')['token'];
$c = $auth->login('two@example.invalid', 'FixturePassword789!')['token'];
check($auth->verifyToken($a) === $id, 'Login failed');
try {
    $auth->changePassword($id, 'weak');
    throw new RuntimeException('Weak password accepted');
...
try {
    ...
    throw new RuntimeException('Failure missing');
} catch (RuntimeException $e) {
    check(str_contains($e->getMessage(), 'fixture failure'), 'Unexpected error');
}
...
try {
    $auth->changePassword(999, 'ReplacementPassword456!');
    throw new RuntimeException('Missing user accepted');
