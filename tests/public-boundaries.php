...
spl_autoload_register(function ($class) use ($core) {
    $prefix = 'Edit\\Core\\';
    if (str_starts_with($class, $prefix)) require $core . str_replace('\\', '/', substr($class, strlen($prefix))) . '.php';
});
...
function addMediaUrl($row) { $row['url'] = '/_edit/uploads/' . $row['path']; return $row; }
