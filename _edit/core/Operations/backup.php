...
function checkDatabase(string $path): void
{
    ...
    if ($foreign->fetchArray()) {
        fail('Database foreign key check failed');
    }
    $db->close();
}
function extractVerified(string $archive, string $stage): array
{
    ...
    if ($zip->open($archive, ZipArchive::RDONLY) !== true) {
        fail('Cannot open archive');
    }
    try {
        if ($zip->numFiles > 100001) {
            fail('Archive has too many entries');
        }
        $raw = $zip->getFromName('manifest.json', 16777217);
        if ($raw === false || strlen($raw) > 16777216) {
            fail('Missing or oversized manifest');
        }
        ...
        for ($i = 0; $i < $zip->numFiles; $i++) {
            ...
            if (!validName($name) || !isset($manifest['files'][$name])) {
                fail('Unexpected archive path');
            }
            $zip->getExternalAttributesIndex($i, $opsys, $attributes);
            $kind = ($attributes >> 16) & 0170000;
            if ($kind !== 0 && $kind !== 0100000) {
                fail('Archive contains non-regular files');
            }
            $total += $stat['size'];
            if ($total > 10737418240) {
                fail('Archive exceeds 10 GiB restore limit');
            }
            $expected = $manifest['files'][$name];
            if (($expected['size'] ?? -1) !== $stat['size'] || !is_string($expected['sha256'] ?? null)) {
            ...
            if (!$input || !$output) {
                fail('Cannot extract archive file');
            }
            $copied = stream_copy_to_stream($input, $output, $stat['size'] + 1);
            fclose($input);
            fclose($output);
            if ($copied !== $stat['size'] || !hash_equals($expected['sha256'], hash_file('sha256', $destination))) {
                fail('Archive checksum mismatch');
            }
        }
        if (count($seen) !== count($manifest['files']) + 1) {
            fail('Archive is incomplete');
        }
        foreach (['config.php', 'core/bootstrap.php', 'admin/index.html', 'api/index.php', 'admin-api/index.php'] as $required) {
            if (!isset($manifest['files'][$required])) {
                fail('Missing required installation file');
            }
        }
        $database = $manifest['database'] ?? '';
        if (!is_string($database) || !validName($database) || !isset($manifest['files'][$database]) || !str_starts_with($database, 'data/')) {
            fail('Invalid database path');
        }
...
function backup(string $source, string $destination): void
{
    ...
    if (!$root || !$parent || !is_dir($root) || inside($parent, dirname($root))) {
        fail('Archive must be outside the website directory, in an existing directory');
    }
    $destination = $parent . '/' . basename($destination);
    if (file_exists($destination) || is_link($destination)) {
        fail('Archive already exists');
    }
    if (!is_file($root . '/config.php') || !is_file($root . '/admin/index.html')) {
        fail('Back up a deployed installation with config.php and built admin assets');
    }
    $stage = sys_get_temp_dir() . '/editcms-backup-' . bin2hex(random_bytes(12));
    $lock = fopen($root . '/data/.operations.lock', 'c');
    if (!$lock) {
        fail('Cannot open operation lock');
    }
    if (!mkdir($stage, 0700)) {
        fclose($lock);
    ...
    try {
        ...
        while (!flock($lock, LOCK_EX | LOCK_NB)) {
            if (microtime(true) > $deadline) {
                fail('CMS is busy; retry backup later');
            }
            usleep(100000);
        }
        ...
        if (!$database || !inside($database, $root . '/data')) {
            fail('Database must be stored inside the installation data directory');
        }
        ...
        try {
            ...
            foreach ($iterator as $entry) {
                ...
                if ($entry->isLink()) {
                    fail('Symlinks are not supported in backups');
                }
                if (!$entry->isFile()) {
                    fail('Non-regular installation file');
                }
                if (in_array($name, ['data/.operations.lock', 'data/.config.lock', 'data/config/.write.lock'], true) || in_array($name, [$relativeDb . '-wal', $relativeDb . '-shm', $relativeDb . '-journal'], true)) {
                    continue;
                }
                $input = $name === $relativeDb ? $snapshot : $path;
                $manifest['files'][$name] = ['size' => filesize($input), 'sha256' => hash_file('sha256', $input)];
                if (!$zip->addFile($input, $name)) {
                    fail('Cannot add archive file');
                }
            }
            if (!$zip->addFromString('manifest.json', json_encode($manifest, JSON_THROW_ON_ERROR | JSON_PRETTY_PRINT))) {
                fail('Cannot add manifest');
            }
        } finally {
            if (!$zip->close()) {
                fail('Cannot finalize archive');
            }
        }
        ...
        if (!link($temporary, $destination)) {
            fail('Cannot publish archive without overwriting');
        }
        unlink($temporary);
...
{
    $parent = realpath(dirname($destination));
    if (!$parent || file_exists($destination) || is_link($destination)) {
        fail('Restore requires a new destination in an existing parent directory');
    }
    $destination = $parent . '/' . basename($destination);
    ...
    try {
        ...
        foreach (['sessions', 'captcha_challenges', 'email_tokens', 'rate_limits'] as $table) {
            if ($db->querySingle("SELECT count(*) FROM sqlite_master WHERE type='table' AND name='{$table}'")) {
                if (!$db->exec("DELETE FROM {$table}")) {
                    fail('Cannot clear restored transient credentials');
                }
            }
        }
        ...
        foreach (['data/config/modules', 'data/config/field-groups', 'data/config/blocks', 'uploads'] as $directory) {
            if (!is_dir($stage . '/' . $directory)) {
                mkdir($stage . '/' . $directory, 0700, true);
            }
        }
        if (file_put_contents($stage . '/data/.restored-database.json', json_encode(['database' => $manifest['database']], JSON_THROW_ON_ERROR)) === false) {
            fail('Cannot write restored database guard');
        }
        if (file_exists($destination) || is_link($destination) || !rename($stage, $destination)) {
            fail('Cannot publish restored installation');
        }
...
try {
    if (!extension_loaded('zip') || !extension_loaded('sqlite3')) {
        fail('PHP CLI requires zip and sqlite3 extensions');
    }
    $command = $argv[1] ?? '';
    if ($command === 'backup' && $argc === 4) {
        backup($argv[2], $argv[3]);
    } elseif ($command === 'restore' && $argc === 4) {
        restore($argv[2], $argv[3]);
    } elseif ($command === 'verify' && $argc === 3) {
        $stage = sys_get_temp_dir() . '/editcms-verify-' . bin2hex(random_bytes(12));
        mkdir($stage, 0700);
        try {
            extractVerified($argv[2], $stage);
            echo "Backup verified\n";
        } finally {
            removeTree($stage);
        }
    } else {
        fail('Usage: php backup.php backup ROOT ARCHIVE | verify ARCHIVE | restore ARCHIVE NEW_ROOT');
    }
} catch (Throwable $e) {
