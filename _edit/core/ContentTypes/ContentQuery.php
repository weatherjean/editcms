...
final class ContentQuery
{
    ...
    public static function field(array $schema, string $key, bool $publicOnly): array
    {
        ...
        if (!in_array($field['type'], self::SCALAR_TYPES, true) || ($field['config']['multiple'] ?? false)) throw new \InvalidArgumentException('Only scalar fields support filtering and sorting');
        return [$path,$field];
    }
    ...
    public static function parse(array $query, array $schema, bool $publicOnly = true): array
    {
        ...
        foreach (['limit'=>[1,100],'offset'=>[0,1000000]] as $key => [$min,$max]) {
            ...
            $value = $query[$key];
            if ((!is_int($value) && !is_string($value)) || !preg_match('/^\d+$/',(string)$value) || $value < $min || $value > $max) throw new \InvalidArgumentException("{$key} must be an integer between {$min} and {$max}");
            $out[$key] = (int)$value;
        }
        foreach (['order_by','order_dir','populate','fields_only','field_groups','exclude_open_fields'] as $key) {
            if (isset($query[$key]) && !is_string($query[$key])) throw new \InvalidArgumentException("{$key} must be a string");
        }
        $out['order_by'] = $query['order_by'] ?? $out['order_by'];
        if (!in_array($out['order_by'], self::CORE_SORTS, true)) [$out['order_by']] = self::field($schema, $out['order_by'], $publicOnly);
        $out['order_dir'] = strtoupper($query['order_dir'] ?? $out['order_dir']);
        if (!in_array($out['order_dir'], ['ASC','DESC'], true)) throw new \InvalidArgumentException('order_dir must be ASC or DESC');
        $out['populate'] = $query['populate'] ?? '';
        $filters = $query['fields'] ?? [];
        if (!is_array($filters) || count($filters) > 20) throw new \InvalidArgumentException('fields must contain at most 20 filters');
        foreach ($filters as $key => $value) {
            if (!is_string($key) || !is_scalar($value) || is_bool($value)) throw new \InvalidArgumentException('Field filters must contain scalar values');
            $operator = '=';
    ...
    {
        if (!in_array($operator, ['=','!=','>=','<=','LIKE'], true) || !is_scalar($value)) throw new \InvalidArgumentException('Invalid field filter');
        if (in_array($field['type'], ['number','boolean','relationship'], true)) {
            if ($operator === 'LIKE' || NumberField::parse($value) === null) throw new \InvalidArgumentException('Numeric filter requires a finite number and a comparison operator');
            $value = NumberField::parse($value);
            if ($field['type'] === 'boolean' && !in_array($value,[0,1],true)) throw new \InvalidArgumentException('Boolean filter must be 0 or 1');
        }
        return $value;
    }
