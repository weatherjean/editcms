...
final class ContentSchema
{
    public static function fields(array $schema, bool $publicOnly = false): array
    {
        ...
        if (!empty($schema['field_groups'])) {
            ...
            foreach ($schema['fields'] ?? [] as $key => $field) $out[$field['key'] ?? $key] = $field;
        }
        if ($schema['allow_open'] ?? false) $out['flexible_content'] = $schema['fields']['flexible_content'] ?? ['key'=>'flexible_content','type'=>'flexible_content'];
    ...
    public static function resolve(array $schema, string $key, bool $publicOnly = false): array
    {
        ...
        if (count($matches) !== 1) throw new \InvalidArgumentException("Unknown or ambiguous field '{$key}'; use group.field for grouped fields");
        $path = array_key_first($matches);
        if ($publicOnly && !isset(self::fields($schema, true)[$path])) throw new \InvalidArgumentException('Field is not available for public queries');
