...
final class PublicSerializer
{
    ...
    public function serialize(array $item, array $populate = [], int $depth = 0): ?array
    {
        ...
        if (!$schema || ($schema['public'] ?? true) !== true || $item['status'] !== 'published') {
            return null;
        }
        ...
        if (!$grouped) {
            $out['fields'] = $this->fields($fields, $schema['fields'] ?? [], $populate, $depth);
        }
    ...
    private function fields(array $values, array $definitions, array $populate, int $depth): array
    {
        ...
        foreach ($definitions as $index => $field) {
            $key = $field['key'] ?? $index;
            if (($field['public'] ?? true) !== true || !array_key_exists($key, $values)) {
                continue;
            }
            $value = $values[$key];
            $type = $field['type'] ?? '';
            if ($type === 'relationship') {
                ...
                if (($field['target'] ?? $field['config']['target'] ?? 'content') === 'user') {
                    continue;
                }
    ...
    private function blocks(mixed $value, array $populate, int $depth): array
    {
        ...
        foreach ($value as $block) {
            ...
            if (!$schema || ($schema['public'] ?? true) !== true) {
                continue;
            }
    ...
    private function relationship(mixed $id, bool $expand, array $populate, int $depth): ?array
    {
        ...
        if (!$expand) {
            return array_intersect_key($row, array_flip(['id', 'type', 'slug']));
        }
