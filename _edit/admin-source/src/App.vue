      ...
      <div class="card w-96 bg-base-100 shadow-xl">
        <div class="card-body gap-4">
          ...
          <form v-else @submit.prevent="handleRegister" class="space-y-4">
            ...
            <fieldset class="fieldset">
              <legend class="fieldset-legend">Setup code</legend>
              <input type="password" v-model="registerForm.setup_code" required autocomplete="off" class="input w-full" />
              <p class="text-sm opacity-60">Enter the one-time code provided by the server administrator.</p>
            </fieldset>
  ...
  <div v-else class="flex flex-col min-h-screen">
    ...
    <div class="navbar bg-base-100 border-b shadow-sm">
      ...
      <div class="flex flex-none gap-0.5">
        <button @click="router.push('/media')" class="btn btn-ghost gap-2" :class="{ 'btn-active': route.name === 'media' }">
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-5">
            <path stroke-linecap="round" stroke-linejoin="round" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" />
          </svg>
          <span class="hidden sm:inline">Media</span>
        </button>
        <button v-if="user?.role === 'admin'" @click="router.push('/users')" class="btn btn-ghost gap-2" :class="{ 'btn-active': route.name === 'users' }">
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-5">
            <path stroke-linecap="round" stroke-linejoin="round" d="M15 19.128a9.38 9.38 0 0 0 2.625.372 9.337 9.337 0 0 0 4.121-.952 4.125 4.125 0 0 0-7.533-2.493M15 19.128v-.003c0-1.113-.285-2.16-.786-3.07M15 19.128v.106A12.318 12.318 0 0 1 8.624 21c-2.331 0-4.512-.645-6.374-1.766l-.001-.109a6.375 6.375 0 0 1 11.964-3.07M12 6.375a3.375 3.375 0 1 1-6.75 0 3.375 3.375 0 0 1 6.75 0Zm8.25 2.25a2.625 2.625 0 1 1-5.25 0 2.625 2.625 0 0 1 5.25 0Z" />
          </svg>
          <span class="hidden sm:inline">Users</span>
        </button>
        <button v-if="user?.role === 'admin'" @click="router.push('/config')" class="btn btn-ghost gap-2" :class="{ 'btn-active': route.name === 'config' }">
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-5">
        ...
        <button v-if="user?.role === 'admin'" @click="router.push('/email')" class="btn btn-ghost gap-2" :class="{ 'btn-active': route.name === 'email' }">
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-5">
        ...
        <div class="dropdown dropdown-end">
          ...
          <ul tabindex="0" class="menu dropdown-content bg-base-100 rounded-box z-[1] mt-3 w-52 p-2 shadow">
            ...
            <li>
              <a @click="toggleTheme">
                <svg v-if="!isDarkMode" xmlns="http://www.w3.org/2000/svg" class="size-4" viewBox="0 0 20 20" fill="currentColor">
...
const registerForm = ref({ name: '', email: '', password: '', setup_code: '' })
...
async function handleRegister() {
  authError.value = null
  try {
    await register(registerForm.value.name, registerForm.value.email, registerForm.value.password, registerForm.value.setup_code)
    registerForm.value = { name: '', email: '', password: '', setup_code: '' }
    showRegister.value = false
...
async function init() {
  if (isAuthenticated.value) {
    ...
    if (route.path === '/' || route.path === '' || (user.value?.role === 'editor' && ['/config','/users','/email'].includes(route.path))) {
      if (sortedPostTypes.value.length > 0) {
        router.push(`/${sortedPostTypes.value[0].key}`)
      } else {
        router.push(user.value?.role === 'admin' ? '/config' : '/media')
      }
    }
