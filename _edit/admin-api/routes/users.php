...
function handleUserRoutes(string $method, string $path, Database $db, Auth $auth, int $userId): bool
{
    ...
    if ($path === '/users' && $method === 'POST') {
        ...
        try {
            ...
            if (!is_string($role)) {
                sendError('Role must be a string', 400);
            }
    ...
    if (preg_match('#^/users/(\d+)/role$#', $path, $matches) && $method === 'PUT') {
        ...
        try {
            ...
            sendError('Unable to change role', 500);
        }
    ...
    if (preg_match('#^/users/(\d+)$#', $path, $matches) && $method === 'PUT') {
        ...
        if (strlen($raw) > 8192) {
            sendError('Password request is too large', 413);
        }
        try {
        ...
        if (!is_array($data) || !is_string($data['password'] ?? null)) {
            sendError('Password must be a string', 400);
        }
        try {
